...
namespace Joestar {
	...
	GPUProgramVulkan::GPUProgramVulkan() {
		...
		vb->SetSize(sizeof(vertices[0]) * vertices.size());
	}
	...
	VkPipelineVertexInputStateCreateInfo* GPUProgramVulkan::GetVertexInputInfo() {
		return vb->GetVKVertexInputInfo();
	}
	...
	void GPUProgramVulkan::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
	...
	void GPUProgramVulkan::CreateTextureImage() {
		Image* img = new Image("texture.jpg");
		...
		CreateImage(img->GetWidth(), img->GetHeight(), VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage, textureImageMemory);
	...
	void GPUProgramVulkan::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
		VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
	...
	VkImageView GPUProgramVulkan::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags) {
